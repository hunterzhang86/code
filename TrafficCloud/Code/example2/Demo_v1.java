...
public class Demo_v1 {
    ...
    //交通拓扑结构表,格式为：TrafficLightID:[FromID1,ToID1(左转）,ToID2(右转）,ToID3（直行）]
	...
	private static int TIME_I = 0;
  	...
  	//使用随机搜索方式对每个交通路口进行状态搜索的次数
	private static int SEARCH_NUM = 200;
	...
	private static void generateStatus() throws IOException{
		//十字路口最好的状态
		Map<String, Integer[]> best_status = new HashMap<String, Integer[]>();
		// 当前十字或者T字路口各个路口灯的状态
		Map<String, Integer[]> current_traffic_status = new HashMap<String, Integer[]>();
		//遍历每个交通路口（十字路口或者T字路口）
		...
		while (iter1.hasNext()) {
			...
			//对每个十字路口使用随机搜索search_num次，选择最好的一次
			for (int search_i = 0; search_i < SEARCH_NUM; ++search_i) {
				current_traffic_status.clear();
				for(String from_id:from_ids){
					//有路的方向
					if(!"#".equals(from_id)){
					   //构造key
					   String key = traffic_light_id+"-"+from_id;
					   //该路口可以通向的三个方向，分别为左转、右转、直行
					   String[] destinations = traffic_light_topo_table.get(key);
					   ...
					   for (int idx = 0; idx < 3; ++idx) {
    						if ("#".equals(destinations[idx])) {
    							...
    							if(idx==1){
    								...
    								//随机，50%概率为绿灯
        							status[idx] = randomStatus();
    							}
    							
    						}
    					}
					   //存储当前得到的转态
					   current_traffic_status.put(key, status);
					}
				}
				//更新状态表
